...
the numbers. So those will be valuable later, and can be pulled out thus:
...
If we use Split on that, we can pull out each direction as a mask, and select
over the numbers. Then the calculation for the first star is
straightforward. The horizontal distance is the sum of all forward movements,
and the depth is the sum of downs minus the sum of ups.
...
I do want to clean that up before going on to the second part.
